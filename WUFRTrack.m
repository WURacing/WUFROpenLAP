...
%
% OpenTRACK
% Modified by Ben Cook to directly work with AIM files
% YAW MODE DOES NOT WORK!!!!! THE CONFIG MUST BE CHANGED FIRST
%
% Track model file creation for use in OpenLAP.
% Instructions:
% 1) Take an AIM CSV export and delete extraneous data [before/after lap].
%    Load that modified CSV in as input when hitting "Run".
...
filename = uigetfile('*.csv', 'Select a Track Sheet', './sheetsTrack') ;
...
if strcmp(mode,'logged data')
    ...
    %% from logged data
    fullname = fullfile('./sheetsTrack', filename);
    [head,data] = read_logged_data(fullname) ;
    info.name = head(4,2);
    info.country = "probably america";
    info.city = head(2,2);
    info.type = "probably a temporary track unless we're really rich";
    info.config = track_config;
    info.direction = track_direction;
    info.mirror = track_mirror;
    % frequency
    ...
    all_channels = [2, 2, 1, 4, 9, 6, 7, 8];
    % channels
    channels = head(15,all_channels) ;
    units = head(16,all_channels) ;

    channels(1) = "Distance";
    channels(6) = "Banking";
    channels(7) = "Grip Factor";
    channels(8) = "Sector";
    ...
    speed_unit = units(1);
    if extractAfter(speed_unit,'/') == "h"
        units(1) = extractBefore(speed_unit,'/');
        for i = 1:height(v)-1
            x(i + 1) = x(i) + v(i) * timestep / 3600;
        end
    else
        disp("UNITS ARE BROKEN")
    end
...
[folder_status,folder_msg] = mkdir('OpenTRACKTracks') ;
trackname = "OpenTRACKTracks/OpenTRACK_"+info.name+"_"+info.config+"_"+info.direction ;
