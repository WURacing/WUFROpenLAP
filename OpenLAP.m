...
%    "OpenLAPSims". You can choose to include the date and time of each
...
trackfile = uigetfile('*.mat', 'Select a Track Sheet', './OpenTRACKTracks') ;
vehiclefile = uigetfile('*.mat', 'Select a Vehicle File', './OpenVEHICLEVehicles') ;

if isequal(trackfile, 0)
    return;
end

if isequal(vehiclefile, 0)
    return;
end
%% Loading circuit
...
simname = "OpenLAPSims/OpenLAP_"+char(veh.name)+"_"+tr.info.name+date_time ;
logfile = simname+".log" ;

%% HUD

[folder_status,folder_msg] = mkdir('OpenLAPSims') ;
delete(simname+".log") ;
...
%% Displaying laptime & Event points
%disp(['Efficiency Factor:  ',num2str(sim.eff.data,'%3.3f')])
%disp(['Efficiency Factor Min:  ',num2str(sim.effMin.data,'%3.3f')])
%disp(['Efficiency Factor Max:  ',num2str(sim.effMax.data,'%3.3f')])
disp(['Efficiency Score:  ',num2str(sim.effScore.data,'%3.3f'),' [pts]'])
fprintf(logid,'%s','Efficiency Score  : ');
fprintf(logid,'%7.3f',sim.effScore.data);
fprintf(logid,'%s\n',' [s]');
